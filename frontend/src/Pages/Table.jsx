      ...
      {open && status === "Booked" && (
        <div className="fixed inset-0 flex flex-col space-y-14 bg-opacity-60 bg-black justify-center items-center w-full h-full p-4 text-white">
          <div className="bg-gray-900 grid space-y-3 p-6 rounded-lg">
            <Link to={`/POS/${order}`} className="text-2xl ">
              Edit Menu
  ...
  return (
    <>
      <div onClick={() => setIsOpen(true)}>
        <button className="bg-cyan-600 p-2 w-48 rounded-xl my-2 text-white text-xl">
          Delivery
        </button>
      </div>
      {open && (
        <div className="fixed inset-0 flex flex-col bg-opacity-50 bg-black justify-center items-center w-full h-full p-4 text-white">
          <div className="rounded-lg  space-y-5 bg-gray-900 ">
            ...
            {newOpen ? <NewCustomerForm /> : <ExistingForm />}
          </div>
          <button
            onClick={() => {
              setIsOpen(false);
...
function ExistingForm({ _id }) {
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    if (_id) {
      const doc = await fetch(`http://localhost:3000/api/createOrderTest`, {
        method: "POST",
        body: JSON.stringify({
          customer: selectedId,
          table: _id,
        ...
        .then((data) => {
          ...
          navigate(`/POS/${data.data._id}`);
        });
    } else {
      const doc = await fetch(`http://localhost:3000/api/createOrder`, {
        method: "POST",
        body: JSON.stringify({
          customer: selectedId,
          orderType: "delivery",
  ...
  return (
    ...
    >
      ...
      >
        <option value="NoCustomerSelected">Select Customer</option>
      ...
      <button type="submit" className="bg-green-500 rounded-lg p-3">
        Select Menu
      </button>
    </form>
  );
