...
// import { order, Menu , Orders} from "../Components/MenuApi";
export default function () {
  ...
  useEffect(() => {
    if (orderType == "all") {
      setUpdatedList(OrdersArray);
      setButton(orderType);
    } else {
      console.log("ordertype", orderType, updatedList);
      setUpdatedList(
        OrdersArray.filter((item) => item.orderType === orderType)
      );
  ...
  const handleClick = (orderType) => {
    ...
    //   setButton(orderType);
    // } else {
    //   const updatedItem = OrdersArray.filter(
    //     (item) => item.orderType == orderType
    //   );
    //   setUpdatedList(updatedItem);
    //   setButton(orderType);
    // }
  };
...
function KOrder({ order, listenerm, setListener }) {
